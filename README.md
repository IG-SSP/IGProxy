# IGProxy

**IGProxy** — установщик и панель управления Telegram MTProxy на базе ядра `telemt`. Проект ставит на VPS прокси, сайт-маскировку, Telegram-бота, локальную web-админку, сбор статистики и бэкапы.

Главная идея: снаружи сервер выглядит как обычный HTTPS-сайт, а Telegram-клиенты используют тот же домен/порт для MTProxy. Управлять можно из SSH-меню, Telegram-бота или локальной web-панели через SSH-туннель.

## Что Входит

- ядро `telemt` с fake-TLS маскировкой;
- CLI-меню `gotelegram`;
- режим «только прокси» без домена и режим «свой домен и сайт» с Let's Encrypt;
- один Telegram-бот с разделением доступа: полное меню для администраторов и прямые кнопки подключения к серверам для клиентов;
- роли «центр» и «дополнительный узел» с одноразовым HTTPS-подключением и автоматическим появлением серверов в едином боте;
- настраиваемые название проекта, спонсорский канал и подписи серверов;
- локальная web-админка на `127.0.0.1:1984`;
- глобальная и per-key история трафика;
- ручные и scheduled бэкапы;
- безопасный выбор свободного публичного порта без перемещения чужих служб.

## Установка

Для обычной ручной установки скопируйте на сервер единый файл `IGProxy`.
При обновлениях заменяйте его новым файлом с тем же именем:

```bash
chmod +x IGProxy
sudo ./IGProxy
```

Сначала скачайте `bootstrap.sh` и релизный архив из одной опубликованной версии.
Сверьте SHA-256 bootstrap по карточке релиза, затем запускайте под `root`:

```bash
GOTELEGRAM_RELEASE_URL="https://example.org/gotelegram-release.tar.gz" \
GOTELEGRAM_RELEASE_SHA256="<64 символа SHA-256 архива>" \
bash bootstrap.sh
```

`bootstrap.sh` проверит архив и внутренний манифест, атомарно переключит
`/opt/gotelegram/current`, создаст `/usr/local/bin/gotelegram` и откроет русское меню.
Для приватного хранилища токен передаётся только через root-only файл
`GOTELEGRAM_TOKEN_FILE`, а не через командную строку.

До установки сервер можно проверить без изменений:

```bash
gotelegram --check
gotelegram --check-json
gotelegram --dry-run
```

`--dry-run` проводит через выбор варианта, порта и DNS и показывает будущие
изменения, но не устанавливает пакеты и не перезапускает службы.

## Варианты установки

Сначала мастер предложит роль. Для первого VPS выбирайте **«Центр управления +
прокси»**. На остальных — **«Дополнительный прокси-узел»** и введите
одноразовый код, созданный на центре в `gotelegram → Управление → Сеть
серверов`. Уже установленный IGProxy обновляется в прежней роли автоматически;
ключи пользователей и дополнительные настройки сохраняются.

**Только прокси** работает без собственного домена. Мастер предлагает `443`, а
если он занят — безопасный альтернативный порт. Чужие службы не перемещаются.

**Свой домен и сайт** требует корректные A/AAAA/CAA-записи. Telegram-клиенты и
обычные браузеры используют выбранный публичный порт: Telegram-трафик идёт в
`telemt`, браузер открывает сайт-маскировку через локальный nginx.

## Web-Админка

Админка слушает только локально на сервере:

```text
127.0.0.1:1984
```

Открой через SSH-туннель:

```bash
ssh -L 1984:127.0.0.1:1984 root@SERVER_IP
```

Потом в браузере:

```text
http://127.0.0.1:1984/
```

Внутри есть сводка, сервисы, проверка сайта и портов, плавные графики трафика,
карточки ключей, QR, IP-лимиты, защищённые бэкапы и понятный режим логов.

## Документация

- [DOCS_HUMAN.md](DOCS_HUMAN.md) — человеческое руководство с нуля.
- [DOCS_AI.md](DOCS_AI.md) — подробная техническая карта для ИИ-агентов и разработчиков.
- [gotelegram-bot/README.md](gotelegram-bot/README.md) — заметки по Telegram-боту.

## Проверки Перед Пушем

Перед пушем:

```bash
bash -n bootstrap.sh install.sh install_gotelegram_bot.sh lib/*.sh
python3 -m unittest discover -s tests -v
python3 tools/build_release.py
PYTHONPYCACHEPREFIX=/tmp/gotelegram-pycache python3 -m py_compile gotelegram-bot/bot.py admin-web/server.py
node --check admin-web/static/app.js
git diff --check
```
