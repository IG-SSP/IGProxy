...
Стабильный релиз с отлаженным под прямой ТСПУ конфигом.

Что меняется (для существующих установок — миграция автоматическая при повторном `bootstrap.sh`):

- **telemt-ядро обновляется** до latest upstream (сейчас 3.4.15). Главное — поддержка `client_mss = "tspu"`, anti-ТСПУ MSS clamping.
- **`/etc/telemt/config.toml` перегенерируется** с правильными параметрами маскировки: `mask_shape_hardening`, `mask_timing_normalization`, `server_hello_delay_min_ms = 20`, `[censorship.tls_fetch] profiles`. Без флага `mask_shape_above_cap_blur` и без `tls_new_session_tickets` — они ломали реальные Telegram-клиенты в наших тестах.
- **`public_host` в `[general.links]`** — ссылки в `tg://proxy?...` теперь указывают на домен, не на IP.
- **`user_max_unique_ips_global_each = 13`** — лимит уникальных IP на пользователя (на случай VPN-pool).
- **Все ключи (`[access.users]`) сохраняются** — миграция извлекает их из старого конфига и переносит в новый.

Откат: бэкап старого config попадёт в `/etc/telemt/config.toml.bak.<timestamp>`, plain restore из `gotelegram` menu → Manage → Restore.
